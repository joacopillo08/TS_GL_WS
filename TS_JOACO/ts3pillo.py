...
# TS2.h = TS2.h - TS2.h.mean() ## aca le saco la media
# TS2.h = TS2.h / np.sqrt(np.var(TS2.h)) ## aca lo divido por la des estandar
# var_h = np.var(TS2.h) ## calculo la varianza
freqs = np.fft.fftfreq(N, Ts)  
...
# --- h ---
plt.subplot(3, 1, 1)
plt.plot(freqs, 20*np.log10(Xhabs1+1e-12), label='|H| de h')
plt.title('h (sistema en diferencias): respuesta en frecuencia')
plt.ylabel('Magnitud [dB]')
plt.grid(True); plt.legend()

# --- h2 (FIR) ---
plt.subplot(3, 1, 2)
plt.plot(freqs, 20*np.log10(Xhabs2+1e-12), label='|H₂| (FIR: x[n]+3·x[n−10])')
plt.title('h2 (FIR, retardo 10): respuesta en frecuencia')
plt.ylabel('Magnitud [dB]')
plt.grid(True); plt.legend()

# --- h3 (IIR inestable) ---
plt.subplot(3, 1, 3)
plt.plot(freqs, 20*np.log10(Xhabs3+1e-12), label='|H₃| (IIR: x[n]+3·y[n−10], inestable)')
plt.title('h3 (IIR, inestable): respuesta en frecuencia')
plt.xlabel('Frecuencia [Hz]')
plt.ylabel('Magnitud [dB]')
plt.grid(True); plt.legend()
